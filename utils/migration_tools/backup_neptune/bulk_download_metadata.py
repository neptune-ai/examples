...
# %%
download_folder = input("Enter the download path (will be created if it doesn't exist): ").strip()
...
# %% Start backup
for project in tqdm(selected_projects, desc="Projects"):
    ...
    with neptune.init_project(project=project, mode="read-only") as _project:
        ...
        for run_id in tqdm(runs, desc="Runs"):
                ...
                for namespace in namespaces:
                    ...
                    try:
                        if str(run[namespace]).split()[0] == "<Artifact":
                            ...
                            os.makedirs(os.path.dirname(namespace_download_path), exist_ok=True)
