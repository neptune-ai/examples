...
class PaperTrader:
    ...
    def load_metadata(self):
        ...
        try:
            with open(
                os.path.join(self.model_config["model_dir"], "metadata.json")
            ) as json_file:
    ...
    def load_res_data(self):
        try:
            df_res = pd.read_csv(
                os.path.join(self.model_config["model_dir"], "results.csv")
            )
        except:
    ...
    def save_data(self, df):
        ...
        df.to_csv(
            os.path.join(self.model_configg["res_file_dir"], "results.csv"), index=False
        )
    ...
    def generate_features(self, df):
        ...
        for i in [7, 14, 21]:
            ...
            df[f"MFI_{i}"] = talib.MFI(
                df["high"], df["low"], df["close"], df["volume"], timeperiod=i
            )
    ...
    def predict_movement(self, df):
        ...
        if len(df_res) > self.model_config["recent_trade_num"] + 1:
            ...
            df_temp_res["price_sign"] = np.sign(
                df_temp_res["closed_trade_price"] - df_temp_res["price"]
            )
            ...
            if (
                sum(df_temp_res["correct_trade"])
                / self.model_config["recent_trade_num"]
                < self.model_config["retraining_thr"]
            ):
        ...
        if model == 0:
            try:
                model = XGBClassifier()
                model.load_model(
                    os.path.join(self.model_config["model_dir"], "model_sklearn.json")
                )
            except:
        ...
        df_res.to_csv(
            os.path.join(self.model_config["model_dir"], "results.csv"), index=False
        )

        with open(
            os.path.join(self.model_config["model_dir"], "metadata.json"), "w"
        ) as outfile:
    ...
    def train_model(self):
        ...
        study.optimize(
            self.objective,
            n_trials=self.model_config["optimization_trials"],
    ...
    def get_score(df_res):
        ...
        df_res["hourly_return"] = df_res[
            ["target_return", "target", "prediction"]
        ].apply(
    ...
    def return_dummy_dataset(start_time, length):
        return pd.DataFrame(
            {
                "date": pd.date_range(
                    start_time, start_time + timedelta(hours=length), freq="H"
                ),
